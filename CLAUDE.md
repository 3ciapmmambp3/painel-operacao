...
- **Attachments** go to a **private Supabase Storage bucket `anexos`** (see `db/02_storage_anexos.sql`). Upload is direct from the browser with the anon key — images are compressed client-side first (`comprimirImagem` in `nova-denuncia.html`), then a **durable signed URL** (~10y) is generated and stored in `denuncias.anexos[].link` (jsonb, per item: `{path,nome,tamanho,mime,link}`). The panel and the Ficha PDF open `a.link || a.web_view_link` (the `web_view_link` fallback is only for legacy Drive rows). The old Google-Drive-via-service-account path (`db/EDGE-FUNCTION-drive.md`) is **deprecated** — service accounts have no storage quota on a personal Gmail.
