-- ════════════════════════════════════════════════════════════════════════
-- 92_tta_editar_cancela_pendencia.sql — cancelamento de pendência NO SERVIDOR
--
-- Reforço do 91: ao EDITAR o TTA, a viatura que SAIU da chamada (estava antes e
-- não está mais — equipe a pé ou troca de viatura) tem sua pendência de Ficha
-- de Movimentação cancelada pelo PRÓPRIO banco, dentro de tta_editar_chamada.
-- Assim não depende do navegador (à prova de cache): o front continua chamando
-- mov_pendencia_cancelar como reforço, mas o servidor garante.
--
-- Só mexe em pendência ABERTA e SEM ficha (atendida=false, mov_id null); nunca
-- toca em ficha já lançada. Recria tta_editar_chamada idêntica ao db/22, com um
-- bloco extra de reconciliação antes do return.
--
-- Depende de: 22 (tta_editar_chamada), 91 (mov_pendencias.cancelada),
--             42 (_tta_conflitos), 07 (tta_chamadas), 04 (_sessao_militar).
-- Idempotente. Rodar no SQL Editor depois do 91.
-- ════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- ── Reconciliação de pendências de ficha (à prova de cache) ─────────────
  -- Viatura que SAIU da chamada (estava em v_old_vtr e não está em v_new_vtr):
  -- cancela a pendência ABERTA e SEM ficha desse dia. Não toca em ficha já
  -- lançada (mov_id not null) nem em pendência já atendida.
  update public.mov_pendencias mp
     set atendida = true, cancelada = true
   where mp.dia = v_data
     and mp.atendida = false
     and mp.mov_id is null
     and mp.prefixo = any (
        select p from unnest(v_old_vtr) p
         where p <> '' and p <> all (v_new_vtr)
     );
...
-- ════════════════════════════════════════════════════════════════════════
-- FIM. Ordem no SQL Editor: depois do 91.
-- ════════════════════════════════════════════════════════════════════════
