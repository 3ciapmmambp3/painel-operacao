-- ════════════════════════════════════════════════════════════════════════
-- 67_roster_escopo_nivel.sql
--   Acrescenta `nivel_acesso` ao retorno de militares_roster_escopo, para a
--   Chamada de Instrução ordenar o COMANDANTE DO GRUPAMENTO (perfil admin_gp)
--   em primeiro dentro do seu grupamento. Mesma lógica de escopo do db/45.
--
--   Muda a assinatura de retorno (nova coluna) → precisa DROP + CREATE.
--   Consumidores (Chamada, Missões) leem por nome no JS, então a coluna extra
--   é inofensiva. Idempotente. Rodar no SQL Editor depois do db/45.
-- ════════════════════════════════════════════════════════════════════════
drop function if exists public.militares_roster_escopo(uuid);
