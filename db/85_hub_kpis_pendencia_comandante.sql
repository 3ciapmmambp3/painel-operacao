-- ════════════════════════════════════════════════════════════════════════
-- 85_hub_kpis_pendencia_comandante.sql — aviso de ficha pendente no Meu Dia
-- também para o COMANDANTE da equipe.
--
-- minhas_pendencias_ficha passa a contar pendências onde o militar é o
-- MOTORISTA ou o COMANDANTE (mov_pendencias.comandante_matricula, criado no
-- db/84). Resto do hub_kpis idêntico ao db/83 (card manutenção, ativo, TTA).
-- Idempotente. Rodar depois do 84.
-- ════════════════════════════════════════════════════════════════════════
...
begin
  ...
  select count(*) into v_vtr_baixa
   ...
   where coalesce(v.ativo, true) = true
     ...
     and public._kpi_inclui(v_ve_tudo, v_nivel_pel, v_meu_gp, v_meu_pel,
           (regexp_match(coalesce(v.gp,''),  '(\d+)'))[1]::int,
           (regexp_match(coalesce(v.pel,''), '(\d+)'))[1]::int);
  select count(*) into v_vtr_man
    from public.viaturas v
   where coalesce(v.ativo, true) = true
     and coalesce(v.situacao_operacional,'DISPONIVEL') = 'EM_MANUTENCAO'
     and public._kpi_inclui(v_ve_tudo, v_nivel_pel, v_meu_gp, v_meu_pel,
           (regexp_match(coalesce(v.gp,''),  '(\d+)'))[1]::int,
           (regexp_match(coalesce(v.pel,''), '(\d+)'))[1]::int);
  ...
  -- Meu Dia: fichas de viatura pendentes onde EU sou o motorista OU o
  -- comandante da equipe (a obrigação da ficha vale para os dois).
  select count(*) into v_minhas_pend
    from public.mov_pendencias
   where atendida = false
     and ( regexp_replace(coalesce(motorista_matricula,''),'\D','','g') = v_mat
        or regexp_replace(coalesce(comandante_matricula,''),'\D','','g') = v_mat );
  ...
  return jsonb_build_object(
    ...
    'viaturas_em_manutencao',  v_vtr_man,
...
-- ════════════════════════════════════════════════════════════════════════
-- FIM. Ordem no SQL Editor: depois do 84 (supersede a versão anterior).
-- ════════════════════════════════════════════════════════════════════════
